...
**Amendment (issue #75): not every 429 is transport-class.** A dispatch that dies because the account's usage window is exhausted also surfaces a 429 in its terminal result event, but it is a different failure class with a different remedy: the quota returns on its own at a known `resetsAt`, so the bounded transport backoff (seconds to minutes) is structurally wrong for it (hours). The runner therefore checks the stream's last parseable `rate_limit_event` for `status:"rejected"` **before** the transport rung — a rejected window short-circuits the transport-retry loop without consuming its budget, and a separate window-retry wrapper sleeps until `resetsAt` + 60s slack and re-dispatches (bounded by `RUNNER_WINDOW_RETRY_MAX_WAITS`; giving up records the combined outcome `window-exhausted` with a `type: window` abort flag). Rung 1's "429 → transport crash" verdict above remains the contract only for a 429 **not** accompanied by a rejected `rate_limit_event`. Detection stays structured-only — the rejected event, never the assistant/result text — and unparseable `rate_limit_event` lines (mid-NDJSON-line injection, claude-code#49640) are skipped in the same fail-safe style as the rest of the ladder. `RUNNER_DISABLE_WINDOW_RETRY=1` restores the unamended behavior.
