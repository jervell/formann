...
- The `runner/` directory in the Consumer's repo is the fixed host-side consumer-prompt root. Consumer prompts referenced via the `consumer:` namespace resolve there. This location is a convention, not a configurable path, and is captured in `RUNNER_CONSUMER_PROMPTS_DIR`.
- The default manifest contains one entry (`review → framework:review-and-gate.md`) reproducing the original hardcoded gate stage byte-for-byte; a Consumer who leaves it as installed gets exactly the prior behaviour.
