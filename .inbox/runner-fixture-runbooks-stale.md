...
The operator-attended runbooks shipped with the runner's smoke/demo fixtures (`multi-feature-drain`, `synthetic-drain`) have drifted from the project's current binding and branch conventions. Following them literally either fails outright or silently degrades to "the runner doesn't see the fixture". Three drivers below, all hit on 2026-05-19 during the `/triage` smoke walk for #10.
...
- The walk that surfaced these: `/triage` for #10, artifact at `.runner-state/smoke-runs/2026-05-19-multi-feature-drain.md`, Verification (2) comment on issue #10.
- Affected runbooks: `framework/runner/tests/fixtures/{multi-feature-drain,synthetic-drain}/README.md`.
