...
Automation that drains `ready-for-agent + AFK` queues while you're away. Built on top of the pipeline — same issues, same state machine — but without the maintainer's keystroke as the bottleneck. Triggered manually from any local state, in one of three modes: bare (`framework/runner/run-the-queue.sh`) drains every active feature; `--feature <slug>` narrows to one feature; `--issue <feature>/<NN>` dispatches a single issue. An optional `--model <id>` flag overrides the model for every dispatch in the run. The runner stops when its scope is drained, or on Ctrl-C.
