...
1. **Argument parsing** — three modes, each accepting an optional `--model <id>` flag:
...
- **Outcome classification** — pre-dispatch and post-dispatch snapshots feed the classifier (`classify_outcome` for implement, `classify_item_action` for each post-implement step). The two classifiers treat the dispatch exit code differently:
  - **Implement** — status delta is the source of truth; the exit code is captured to `<NN>.exit` but never reaches the *outcome* classifier (`classify_outcome`). `/implement` has no "non-zero on failure" contract and claude may exit 0 from a session that didn't ship, so the committed `tracker:` flip is the only trustworthy signal. (The exit code does feed the separate *transport-crash* classifier's Rung-2 safety net — a death with no recoverable `result` event — which decides retry-vs-fail, not success-vs-failure.)
...
**Stale-parking-ref sweep.** Once per runner invocation, immediately after pre-flight and before any tracker query or dispatch, the runner sweeps `refs/remotes/runner/*` on host. For each parking ref, it asks git which other refs contain the parking ref's tip commit (`git for-each-ref --contains <tip>`). If another ref (any ref except the parking ref itself and `refs/remotes/runner/HEAD`) contains the tip, **and** the runner-checkout's `refs/heads/<slug>` tip is similarly reachable from a host ref (when the source branch is present), both deletions happen: the runner-checkout's source branch first, then the host parking ref. Source-first order closes a race where an interleaved `git fetch runner` would restore the host ref from the surviving source. If either reachability check fails, the slug is left untouched and self-cleans on the next successful propagate. If the source-side delete itself fails (lock contention, permissions), the host parking ref is also kept — proceeding alone would reintroduce the original cluttering bug. The sweep is safe-by-construction: deletion only happens when reachability through another ref is proven AND the deletion on the other side succeeded (or wasn't needed). `refs/remotes/runner/HEAD` (the symbolic ref git creates for the runner remote) is never treated as a parking ref and is unconditionally skipped. A parking ref with an unreadable tip is skipped with a warning in `runner.log`; the sweep continues and exits 0. Each deletion is logged with the deleted ref and the witnessing ref. When at least one ref is swept, `SUMMARY.md` gains a `## Swept parking refs` section; when no refs are swept the section is absent. A feature whose parking ref was swept by the runner still dispatches normally on the next run — `ensure_runner_checkout_on_branch` sees an absent parking ref, syncs from the host branch tip, and the first successful propagation recreates the parking ref.
...
3. **Per-feature gate cascade** — `drain_one_feature` runs the side-effecting helpers in short-circuit order and records `drain` or `skip:<reason>` at the first failing step (priority: `fetch-failed` → `feature-snapshot-failed` → `queue-empty` → `drain`). Missing host branches are no longer a gate signal — `ensure_runner_checkout_on_branch` lazily creates the runner-checkout branch from the default branch's current host tip, fetched at sync time, when the host has no ref yet. The default branch is resolved deliberately by `resolve_default_branch` (precedence: the `RUNNER_DEFAULT_BRANCH` override, then the host's own `refs/remotes/origin/HEAD`, then `refs/heads/main`/`refs/heads/master` on host; loud failure naming the knob otherwise) — never from the host's currently-checked-out branch or the runner-checkout's clone-time `origin/HEAD`, both accidents of timing.
...
4. **Implement dispatch** — capture the runner-checkout's HEAD, then run `claude -p "/implement <ref>" --output-format stream-json --verbose --dangerously-skip-permissions` (with `--model <id>` appended when `--model` was passed to the runner) in a fresh sandbox container (no PTY). The dispatch's stdout (the streamed structured-event trace) is captured to `<run-dir>/<NN>.stdout.jsonl` and its stderr to `<NN>.stderr.log`; the agent's closing message is extracted from the terminal `result` event into `<NN>.summary.md`, and the exit code to `<NN>.exit`. Capture HEAD again afterwards.
...
7. **Post-implement walk** — every successful AFK implement proceeds to the manifest walk (`walk_post_implement_steps`). Both run modes refuse non-AFK refs before reaching this point (loop via the snapshot's `eligible` filter, single-dispatch via the same flag in `run_single`), so `dispatch_one` never sees a non-AFK ref. The walk iterates the resolved manifest items in order; for each item:
   - **Dispatch the step** — `claude -p "<step-prompt>\n<ref>" --output-format stream-json --verbose --dangerously-skip-permissions` (with `--model <id>` appended when `--model` was passed to the runner) in a fresh sandbox container (no PTY). Captured to the same per-dispatch artifact set as the implement stage: `<NN>-<step>-<label>.stdout.jsonl` (event stream), `.stderr.log` (diagnostics), `.summary.md` (extracted closing message).
...
The runner classifies each step's outcome from the post-step snapshot and the dispatch exit code (`classify_item_action`) — it does not parse the verdict line. The verdict line exists for the human reading the step's artifacts (`<NN>-<step>-<label>.summary.md` for the skim, `.stdout.jsonl` for the full trace) after the fact.
